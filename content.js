...
function printAndClickSubmitButton(form) {
  ...
  buttons.forEach((button) => {
    if (button.querySelector('span') && button.querySelector('span').textContent.toUpperCase().includes('SUBMIT')) {
      console.log(`Found and clicked Submit Button: ${button.querySelector('span').textContent}`);
      button.click();
      setTimeout(() => {
  ...
  if (request.action === 'startHighlighting') {
    ...
    intervalId = setInterval(() => {
      ...
      document.querySelectorAll('form').forEach((form) => {
        ...
        const hasQuestionText = Array.from(form.querySelectorAll('*')).some((el) => {
          return el.textContent.includes('Question') && el.textContent.includes('of');
        });
        if (hasQuestionText) {
          ...
          const newColor = form.dataset.color === '2px solid red' ? 'blue' : 'red';
