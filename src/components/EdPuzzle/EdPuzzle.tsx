...
function EdPuzzle() {
  ...
  const [clickToAnswer, setClickToAnswer] = useState(false);

  async function onInit() {
    try {
      console.log("onInit");
      await getAssignmentData(await assignment_attempt_webrequest.get());
      console.log("retrievd assignment data")
  ...
  async function unlockTimeline(): Promise<void> {
    console.log('unlocking timeline');
    const data = await assignment_attempt_data.get();
    console.log("reteiving data: " + JSON.stringify(data));
    const version = await edpuzzle_version.get();
    console.log('version: ', version);
    if (timelineUnlocked || (data.timeIntervals.at(-1)?.views ?? 0) > 0) {
      timelineUnlocked = true;
      console.log("timeline already unlocked");
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      chrome.storage.local.get('edpuzzle_clickToAnswer', ({ edpuzzle_clickToAnswer }) => {
        setClickToAnswer(typeof edpuzzle_clickToAnswer === 'boolean' ? edpuzzle_clickToAnswer : false);
      });
      console.log("calling onit")
      await onInit();
  ...
  const handleCheckboxChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setClickToAnswer(enabled);
    chrome.storage.local.set({ edpuzzle_clickToAnswer: enabled });
    const tabs = await chrome.tabs.query({ active: true, currentWindow: true });
    if (tabs[0].id) {
      chrome.tabs.sendMessage(tabs[0].id, Message(MessageAction.Edpuzzle_ClickToAnswer, { enabled }));
    }
  };

  const handleUnlockTimelineClick = async () => {
    unlockTimeline();
  };

  const handleSubmitAnswersClick = async () => {
    const tabs = await chrome.tabs.query({ active: true, currentWindow: true });
    if (tabs[0].id) {
      chrome.tabs.sendMessage(tabs[0].id, Message(MessageAction.Edpuzzle_SubmitAllAnswers, {}));
    }
  };
